...
contract SphynxVaults is Ownable {
  using SafeMath for uint256;
  ...
  IERC20 public erc20; // SPH address
  ...
  function fund(uint256 _amount) public {
    require(block.number < endBlock, 'SPH-Fund: Too late, the farm is closed!');
  ...
  function add(
    ...
    for (uint256 index = 0; index < poolInfo.length; index++) {
      require(
        ...
        'SPH-Add: you can not add the same LP token twice'
      );
    }
  ...
  function withdraw(uint256 _pid, uint256 _amount) public {
    ...
    require(
      user.amount >= _amount,
      "SPH-withdraw: Cannot withdraw more than deposit"
    );
    updatePool(_pid);
    uint256 pendingAmount =
      user.amount.mul(pool.accERC20PerShare).div(1e36).sub(user.rewardDebt);
